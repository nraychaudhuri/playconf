...
public abstract class ServerSentEvent extends Chunks<String> {
	...
	public ServerSentEvent() {
		super(play.core.j.JavaResults.writeString("text/event-stream",
				play.api.mvc.Codec.javaSupported("utf-8")));
	}
	 ...
	 * event name. On the client, an event listener can be setup to listen to
	...
	/**
	 * Setting an ID lets the browser keep track of the last event fired so that
	 * if, the connection to the server is dropped, a special HTTP header
	 * (Last-Event-ID) is set with the new request. This lets the browser
	 * determine which event is appropriate to fire.
	 * 
	 * @param eventId
	 *            Unique event id.
	 * @param data
	...
	/**
	 * Sending a generic event. On the client, 'message' event listener can be
